...
{
    my_left_hand_pub_ = create_publisher<sensor_msgs::msg::JointState>("my_msg", 10);
...
void ManusDataPublisher::PublishCallback()
{
    ...
    for (size_t i = 0; i < m_Landscape->gloveDevices.gloveCount; i++)
    {
        ...
        bool p_Left=true;
        //sensor_msgs::JointState my_temp_msg;
        //ClientLog::printWithPadding
        //RoundFloatValue
        //SDKClient::left_hand_pub_.publish(my_temp_msg);
        ...
        const int TARGET_JOINT_NUM = 10 ;
        float finger_bend_angles[NUM_FINGERS_ON_HAND];
        float thumb_extra_angle = 0.0f;
        // std_my_temp_msgs::Int32MultiArray my_temp_msg; 
        // my_temp_msg.data.resize( TARGET_JOINT_NUM );
        sensor_msgs::msg::JointState my_temp_msg;
        my_temp_msg.header.stamp = this->get_clock()->now();          // 时间戳就在这里
        my_temp_msg.position.resize(TARGET_JOINT_NUM);        // 存 6 维数据（0~1000，原样）
        // （可选）填 name，便于调试/对齐；不想加也可以删掉下面这段
        my_temp_msg.name.resize(TARGET_JOINT_NUM);
        const std::string prefix = p_Left ? "left_" : "right_";
        my_temp_msg.name[0] = prefix + "pinky";
        my_temp_msg.name[1] = prefix + "ring";
        my_temp_msg.name[2] = prefix + "middle";
        my_temp_msg.name[3] = prefix + "index";
        my_temp_msg.name[4] = prefix + "thumb_root";//spread//1
        my_temp_msg.name[5] = prefix + "thumb_ip";//stretch//0

        my_temp_msg.name[6] = prefix + "thumb0";
        my_temp_msg.name[7] = prefix + "thumb1";
        my_temp_msg.name[8] = prefix + "thumb2";
        my_temp_msg.name[9] = prefix + "thumb3";

        my_temp_msg.position[6] = t_ErgoData.data[0];
        my_temp_msg.position[7] = t_ErgoData.data[1];
        my_temp_msg.position[8] = t_ErgoData.data[2];
        my_temp_msg.position[9] = t_ErgoData.data[3];
        ...
        for (unsigned int t_FingerNumber = 0; t_FingerNumber < NUM_FINGERS_ON_HAND; t_FingerNumber++)
        {
            ...
            // t_JointNames[0],
            // RoundFloatValue(t_ErgoData.data[t_DataOffset], 2),
            // t_JointNames[0],
            // RoundFloatValue(t_ErgoData.data[t_DataOffset + 1], 2),
            // t_JointNames[1],
            // RoundFloatValue(t_ErgoData.data[t_DataOffset + 2], 2),
            // t_JointNames[2],
            // RoundFloatValue(t_ErgoData.data[t_DataOffset + 3], 2));

            // my_temp_msg.data[t_FingerNumber]=RoundFloatValue(t_ErgoData.data[t_DataOffset+1], 2);
            // my_temp_msg.data[15+t_FingerNumber]=RoundFloatValue(t_ErgoData.data[t_DataOffset+2], 2);

            finger_bend_angles[t_FingerNumber] = RoundFloatValue(t_ErgoData.data[t_DataOffset + 1], 2);

            // 若为拇指，额外提取ip关节的弯曲角（t_DataOffset + 3）作为第6个数据
            if (t_FingerNumber == 0) {  // t_FingerNumber=0对应拇指
                thumb_extra_angle = RoundFloatValue(t_ErgoData.data[t_DataOffset], 2);
            }

                t_JointNames = t_FingerJointNames;
                t_DataOffset += 4;
        }
        // my_temp_msg.data[5] = thumb_extra_angle;

        // my_temp_msg.data[0]=finger_bend_angles[4];
        // my_temp_msg.data[1]=finger_bend_angles[3];
        // my_temp_msg.data[2]=finger_bend_angles[2];
        // my_temp_msg.data[3]=finger_bend_angles[1];
        // my_temp_msg.data[4]=finger_bend_angles[0];
        int my_i = 0;
        for(my_i=0;my_i<6;my_i++)
        {   
            if(my_i==4)
            {
            // if(abs(finger_bend_angles[0])>80)
            //     my_temp_msg.position[my_i]=80 ;
            // else if(finger_bend_angles[0]<0)
            //     my_temp_msg.position[my_i]=0;
            // else
                my_temp_msg.position[my_i]=static_cast<int>(finger_bend_angles[0]);
            }
            else if(my_i==5)
            {
            // if(abs(thumb_extra_angle)>90)
            //     my_temp_msg.position[my_i]=90 ;
            // else if(thumb_extra_angle<0)
            //     my_temp_msg.position[my_i]=0;
            // else
                my_temp_msg.position[my_i]=static_cast<int>(thumb_extra_angle);
            }
            else
            {
                
                if(abs(finger_bend_angles[4-my_i])>90)
                    my_temp_msg.position[my_i]=90 ;
                else if(finger_bend_angles[4-my_i]<0)
                    my_temp_msg.position[my_i]=180;
                else
                    my_temp_msg.position[my_i]=static_cast<int>(180-finger_bend_angles[4-my_i]);
            }
            //0~3分别为小拇指到食指，4为拇指弯曲，5为拇指摆动,
            //0~3提取到的都是mcp,也就是离手掌最近的关节的弯曲值,发布的是手指与手掌的角度数据，从180开始
            //5是大拇指的摆动值，为与食指的夹角
            //4是拇指弯曲，发布数据为弯曲的角度，从0开始
        ...
        // 发布消息
        if (p_Left) {
            my_left_hand_pub_->publish(my_temp_msg);
        } 
        else 
        {
            //my_right_hand_pub_.publish(my_temp_msg);


            // ROS_INFO("Published joints (小指→无名指→中指→食指→大拇指根→大拇指ip): "
            //     "[%.2f, %.2f, %.2f, %.2f, %.2f, %.2f]",
            //     my_temp_msg.position[0], my_temp_msg.position[1], my_temp_msg.position[2],
            //     my_temp_msg.position[3], my_temp_msg.position[4], my_temp_msg.position[5]);
        }

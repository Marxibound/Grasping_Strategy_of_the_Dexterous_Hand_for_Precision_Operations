...
def _get_pose_by_tf(tf_listener) -> Optional[np.ndarray]:
    """通过TF变换获取位姿"""
    target_frame = "block_top_center"
    source_frame = "base_link"
    try:
        # 等待TF变换
        tf_listener.waitForTransform(
            source_frame, target_frame, rospy.Time(0), rospy.Duration(10.0)
        )
        # 获取变换
        ...
        # 组合为[x, y, z, rx, ry, rz]格式
        return np.array([
            trans[0], trans[1], trans[2],
            roll, pitch, yaw
        ])
    except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException) as e:
        rospy.logerr(f"TF获取位姿失败: {str(e)}")
        return None
...
if __name__ == '__main__':
    ...
    try:
        ...
        while(ops.existstation_flag):
            ...
            # ------------------1-----------------
            run("A S PTP J 0.1 0.1 init_pose")

            run("W 10.0")

            pose_list = _get_pose_by_tf(tf_listener)
            # pose_list[2]
            pose_list[0] += 0.01
            pose_list[2] += 0.22
            yaw = pose_list[5]
            pose_list[3:] = [0.0, 180.0, yaw - 45]
            # print(pose_list)
            script = f"a S PTP P 0.1 0.1 {' '.join(map(str, np.round(pose_list, 6)))}"
            run(script)
            ...
            run("A S PTP J 0.1 0.1 mid_point")

            run("A S PTP J 0.1 0.1 home1")
            run("G -1 1000 0 99999999")
            ...
            # run("a S PTP P 0.1 0.1 -0.063484 0.521 0.49373 0.0 180.0 0.0")
